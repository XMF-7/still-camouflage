...
def draw_box3d_image(image, qs, img_size=(900, 1600), color=(255,255,255), thickness=4):
	...
	if pts_outside >= 7: return image, False
...
def vis_obj(box, img, calib, hw, color_tmp=None, str_vis=None, thickness=4,
			id_hl=None, err_type=None, text_scale=0.5, text_thickness=None):
	...
	if draw and obj_pts_2d is not None and str_vis is not None:
		...
		img = cv2.putText(
			img, str_vis, (x1+5, y1-10), cv2.FONT_HERSHEY_TRIPLEX,
			text_scale, color_tmp, text_thickness
		)
...
def vis_image_with_obj(img, obj_res, obj_gt, calib, hw, save_path, h_thres=0, \
	color_type='det', id_hl=None, repeat=60, text_scale=0.5, text_thickness=None):
	...
	for obj in obj_res:
		...
		if depth >= 2: 		# check in front of camera
			...
			if id_hl is not None and obj.id in id_hl:
				...
				err_type = None
			img = vis_obj(
				box_tmp, img, calib, hw['image'], color_tmp, str_vis,
				thickness, id_hl, err_type, text_scale=text_scale,
