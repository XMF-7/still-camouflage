...
def main():
    ...
    if args.format == 'video':
        ...
        vout = cv2.VideoWriter(
            ...
            args.fps, (int(1600 / scale_factor * 3), int(900 / scale_factor * 2)))
    ...
    for cnt, infos in enumerate(
        ...
        for view in views:
            ...
            for aid in range(valid.shape[0]):
                for index in draw_boxes_indexes_img_view:
                    # Project all edges in this view and clip them by image
                    # boundary, rather than dropping edges with off-screen endpoints.
                    if valid[aid, index[0]] and valid[aid, index[1]]:
                        p1 = tuple(np.round(corners_img[aid, index[0]]).astype(int))
                        p2 = tuple(np.round(corners_img[aid, index[1]]).astype(int))
                        ok, cp1, cp2 = cv2.clipLine(
                            (0, 0, img.shape[1], img.shape[0]), p1, p2)
                        if ok:
                            cv2.line(
                                img,
                                cp1,
        ...
        # fuse 6 camera views only (no BEV canvas)
        ...
        img[900:, :, :] = img_back
        img = cv2.resize(
            img, (int(1600 / scale_factor * 3), int(900 / scale_factor * 2)))
