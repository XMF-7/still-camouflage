...
# The host may provide a newer CUDA toolkit than the CUDA version used by the
# installed PyTorch binary (e.g. toolkit 13.x vs torch cu11.3). In that case
# PyTorch's build helper aborts before compilation. We bypass only this version
# gate to allow compiling BEVDet's custom op in mixed host environments.
BuildExtension._check_cuda_version = lambda self: None
torch_cpp_extension._check_cuda_version = lambda *args, **kwargs: None
