...
def get_color(category_name: str):
    ...
    This method works for the general nuScenes categories, as well as the nuScenes detection categories.
    """
    # nuScenes 标准颜色映射（RGB值）
    colormap = {
        'vehicle.bicycle': [119, 11, 32],
        'vehicle.construction': [0, 0, 255],
        'movable_object.trafficcone': [0, 255, 255],
        'vehicle.car': [255, 158, 0],
        'vehicle.truck': [0, 0, 230],
        'vehicle.bus.bendy': [0, 0, 70],
        'vehicle.bus.rigid': [0, 0, 70],
        'vehicle.trailer': [0, 0, 110],
        'movable_object.barrier': [112, 128, 144],
        'vehicle.motorcycle': [0, 0, 230],
        'human.pedestrian.adult': [220, 20, 60],
        'human.pedestrian.child': [220, 20, 60],
        'human.pedestrian.construction_worker': [220, 20, 60],
        'human.pedestrian.personal_mobility': [220, 20, 60],
        'human.pedestrian.police_officer': [220, 20, 60],
        'human.pedestrian.stroller': [220, 20, 60],
        'human.pedestrian.wheelchair': [220, 20, 60],
    }
    
    # 特殊类别映射（检测类别名称 -> nuScenes 类别名称）
    if category_name == 'bicycle':
        return colormap.get('vehicle.bicycle', [119, 11, 32])
    elif category_name == 'construction_vehicle':
        return colormap.get('vehicle.construction', [0, 0, 255])
    elif category_name == 'traffic_cone':
        return colormap.get('movable_object.trafficcone', [0, 255, 255])
    elif category_name == 'car':
        return colormap.get('vehicle.car', [255, 158, 0])
    elif category_name == 'truck':
        return colormap.get('vehicle.truck', [0, 0, 230])
    elif category_name == 'bus':
        return colormap.get('vehicle.bus.rigid', [0, 0, 70])
    elif category_name == 'trailer':
        return colormap.get('vehicle.trailer', [0, 0, 110])
    elif category_name == 'barrier':
        return colormap.get('movable_object.barrier', [112, 128, 144])
    elif category_name == 'motorcycle':
        return colormap.get('vehicle.motorcycle', [0, 0, 230])
    elif category_name == 'pedestrian':
        return colormap.get('human.pedestrian.adult', [220, 20, 60])
...
def render_sample_data(
        sample_toekn: str,
        with_anns: bool = True,
        box_vis_level: BoxVisibility = BoxVisibility.ANY,  # 使用 BoxVisibility.ANY 最宽松，但仍会过滤完全不在图像内的 box
        axes_limit: float = 40,
        ax=None,
        nsweeps: int = 1,
        out_path: str = None,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            for record in pred_data['results'][sample_toekn]:
                if record['detection_score'] > 0.2:
                    ...
                    # 将score信息附加到box对象上
                    box.detection_score = record['detection_score']
                    boxes_with_scores.append(box)
            ...
            if with_anns:
                for box in boxes_pred:
                    ...
                    # ========== 添加car类别的置信度标签 ==========
                    if box.name == 'car' and hasattr(box, 'detection_score'):
                        try:
                            # 获取box的3D中心点并投影到2D
                            # box.center 是 [x, y, z] 格式的numpy数组
                            center_3d = np.array(box.center).reshape(3, 1)  # [3, 1]
                            center_2d = view_points(center_3d, camera_intrinsic, normalize=True)[:2, 0]  # [2]
                            
                            # 确保2D坐标在图像范围内
                            if 0 <= center_2d[0] < data.size[0] and 0 <= center_2d[1] < data.size[1]:
                                # 在box中心上方显示置信度
                                score_text = f'car: {box.detection_score:.2f}'
                                ax[j, ind].text(center_2d[0], center_2d[1] - 10, score_text,
...
if __name__ == '__main__':
    ...
    bevformer_results = mmcv.load('/home/ads/Code/zz5-3D-BEVFormer/BEVFormer/test/bevformer_small/now/pts_bbox/results_nusc.json')
    
    save_dir = "visual-results/result-2"
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)

    sample_token_list = list(bevformer_results['results'].keys())

    # Keep a deterministic temporal order: sort by nuScenes sample timestamp.
    token_ts_pairs = []
    fallback_tokens = []
    for token in sample_token_list:
        try:
            ts = nusc.get('sample', token)['timestamp']
            token_ts_pairs.append((int(ts), token))
        except Exception:
            # If token is missing in DB, keep it but append at tail.
            fallback_tokens.append(token)
    token_ts_pairs.sort(key=lambda x: x[0])
    sample_token_list = [t for _, t in token_ts_pairs] + fallback_tokens

    for idx, token in enumerate(sample_token_list):
        # Prefix frame index so file-browser sorting matches temporal order.
        out_path = os.path.join(save_dir, f"{idx:03d}_{token}")
