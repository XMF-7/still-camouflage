...
try:  # pragma: no cover - plotting is optional for library use
...
def configure_planner(
    *,
    ...
    stop_s=None,
    d_s=None,
    n_stop_s_sample=None,
):
    global LATERAL_MOVEMENT
    global LONGITUDINAL_MOVEMENT
    global MAX_SPEED
    global MAX_ACCEL
    ...
    global STOP_S
    global D_S
    global N_STOP_S_SAMPLE
    global LATERAL_MOVEMENT_STRATEGY
    global LONGITUDINAL_MOVEMENT_STRATEGY

    if lateral_movement is not None:
        LATERAL_MOVEMENT = lateral_movement
    if longitudinal_movement is not None:
    ...
    if min_t is not None:
        MIN_T = min_t
    if n_s_sample is not None:
        N_S_SAMPLE = n_s_sample
    if k_j is not None:
        K_J = k_j
    if k_t is not None:
        K_T = k_t
    if k_s_dot is not None:
        K_S_DOT = k_s_dot
    if k_d is not None:
        K_D = k_d
    if k_s is not None:
    ...
    if k_lon is not None:
        K_LON = k_lon
    if max_road_width is not None:
        MAX_ROAD_WIDTH = max_road_width
    if d_road_w is not None:
        D_ROAD_W = d_road_w
    if target_speed is not None:
        TARGET_SPEED = target_speed
    if d_t_s is not None:
        D_T_S = d_t_s
    ...
    if d_s is not None:
        D_S = d_s
    if n_stop_s_sample is not None:
        N_STOP_S_SAMPLE = n_stop_s_sample
